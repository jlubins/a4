...
<a href="/post/{{ $post['id'] }}/delete">Delete Post</a> @endif @endforeach
<br>
<br> @if($replies) @foreach($replies as $reply)
<h4>{{ $reply->content }} </h4>
<h5> by {{ $user2[0]['name'] }} </h5> @if($match2)
<a href="/post/{{ $reply['id'] }}/edit">Edit Post</a>
<a href="/post/{{ $reply['id'] }}/delete">Delete Post</a> @endif
<br> @endforeach @endif
